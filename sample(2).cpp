...
const char *WINDOWTITLE = { "Particle System -- Joshua Strozzi" };
...
const float DT = 	{ 0.010f };			//Turned down from .03 to .01
const float GRAVITY = 	{ -50. };		//Turned up from -10.
...
Animate( void )
{
	...
	for( i=0, p=Fire; i < NUMFIRES; i++, p++ )
	{
		dt = Time - p->t0;
		p->x = p->x +(p->vx * dt);
		p->y = p->y +(p->vx * dt) + (.5*GRAVITY*dt*dt);
		p->z = p->z +(p->vz * dt);
	}
...
InitGraphics( void )
{
	...
	for( i=0, p=Fire; i < NUMFIRES; i++, p++ )
	{
		p->x0 = Ranf(XMIN, XMAX);					//Set particle start x pos
		p->y0 = Ranf(YMIN, YMAX);					//Set particle start y pos
		p->z0 = Ranf(ZMIN, ZMAX);					//Set particle start z pos
		/*
		p->x0 = 100*sin( Ranf(THETAMIN, THETAMAX) );					//Set particle start x pos
		p->y0 = 100*sin(Ranf(THETAMIN, THETAMAX));					//Set particle start y pos
		p->z0 = 100*sin(Ranf(THETAMIN, THETAMAX));					//Set particle start z */
		p->x = p->x0;								//place curr x at start x
		p->y = p->y0;								//place curr y at start y
		p->z = p->z0;								//place curr z at start z
		float v = 0.;
		float th = 0.;
		p->vx = Ranf(VELMIN, VELMAX);				//Set x direction vel
		p->vy = Ranf(VELMIN, VELMAX);				//Set y direction vel
		p->vz = Ranf(VELMIN, VELMAX);				//Set Z direction vel
		p->t0 = Ranf(TMIN, TMAX);					//Set particle spawn time
		p->t1 = p->t0 + Ranf(TMIN, TMAX);			//Set Particle death time
		p->rad = Ranf(RMIN, RMAX);					//Set radius of particle
		p->r = Ranf(0., 1.);						//Set Red val
		p->g = Ranf(0., 1.);						//Set green val
		p->b = Ranf(0., 1.);						//Set blue val
		p->ti = 0;									//Set number of previous positions to keep track of
